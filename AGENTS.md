...
- Codex 0.129.0 also persists per-hook review decisions under `[hooks.state]` in `~/.codex/config.toml`; a missing Speak Swiftly entry there means the operator needs to approve the plugin-managed `Stop` hook in Codex settings before it runs.
