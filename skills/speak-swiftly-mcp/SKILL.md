...
- For a broad status question, read `speak://runtime/overview` first. Use `get_runtime_overview` only for compatibility clients that cannot read MCP resources cleanly.
...
- When a tool returns `request_id`, follow it with `speak://requests/{request_id}` or `speak://runtime/overview` instead of guessing whether the work finished.
