...
- Codex 0.129.0 stores per-hook review decisions under `[hooks.state]` in `~/.codex/config.toml`; if Codex says hooks need review, use the hooks settings panel to approve the Speak Swiftly `Stop` command instead of adding a user-level fallback hook.
...
- Plugin-managed hook commands must target the installed Socket Codex cache payload path at `~/.codex/plugins/cache/socket/speak-swiftly/11.0.0-alpha.2/hooks/...`. Do not keep stale standalone `SpeakSwiftlyServer` cache commands in the Socket-managed manifest.
...
- Warn when the expected `speak-swiftly@socket:hooks/hooks.json:stop:0:0` entry is missing from `[hooks.state]`; that means the new Codex per-hook review gate probably still needs operator approval.
...
- Run `node scripts/codex-hooks-doctor.mjs --repair-plan` and inspect both centralized and repo-local recent stop-hook log summaries.
