...
Runtime model selection is startup-only as well. Persisted runtime configuration and the matching HTTP/MCP surfaces use `speech_backend`; callers choose exact Qwen and Marvis variants through that backend identifier rather than separate resident-model or resident-policy knobs. `SPEAKSWIFTLY_SPEECH_BACKEND` overrides the persisted next-start value while building the explicit `SpeakSwiftly.Configuration` passed into runtime startup.
...
`POST /speech/live` mirrors the current public live-speech queue lane and accepts optional `profile_name`, `cwd`, `repo_root`, `request_context`, `text_profile_id`, and `qwen_pre_model_text_chunking` fields so callers can pass voice, path-aware caller metadata, stored text normalization, and Qwen live-chunking context explicitly. When `profile_name` is omitted, the server uses the configured app default voice when one exists, then falls back to the runtime's built-in default voice. HTTP speech routes fill default `request_context` provenance for the HTTP surface, method, route, server identity attributes, and speech topic; caller-provided `request_context` fields override those defaults, while `cwd` and `repo_root` still merge into the shared context. `qwen_pre_model_text_chunking` is an opt-in boolean for Qwen live playback only; omitted requests keep SpeakSwiftly's default single-pass Qwen live path.
...
- `GET /configuration` and `PUT /configuration` expose saved next-start runtime configuration. This is startup intent, not a live transition feed. The current transport field is `speech_backend`; `speech_backend` can also be switched live through `POST /backend`.
