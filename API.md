...
The current package depends on `SpeakSwiftly` from `9.0.2`, `TextForSpeech` from `0.22.0`, Hummingbird from `2.21.1`, the Swift MCP SDK from `0.12.0`, Swift Configuration from `1.2.0`, Swift Async Algorithms from `1.1.3`, and `mlx-swift-lm` exact `3.31.3`.
