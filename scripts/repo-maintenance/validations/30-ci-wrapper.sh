...
if grep -Fq "scripts/repo-maintenance/validate-all.sh" "$workflow_path"; then
  die "Expected $workflow_path to keep scripts/repo-maintenance/validate-all.sh local and call scripts/repo-maintenance/validate-ci.sh instead."
fi
