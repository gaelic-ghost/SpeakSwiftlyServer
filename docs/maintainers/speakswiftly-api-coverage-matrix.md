...
| `SpeakSwiftly.Configuration.qwenResidentModel` | Full | `GET /configuration`, `PUT /configuration` with `qwen_resident_model` | `set_runtime_configuration` with `qwen_resident_model`, `speak-swiftly://configuration` | Startup-only configuration. Accepts `base_0_6b_8bit` and `base_1_7b_8bit`, reports both active and next-start values, and honors the upstream `SPEAKSWIFTLY_QWEN_RESIDENT_MODEL` override when building the startup configuration passed to `SpeakSwiftly.liftoff(configuration:)`. |
...
| `runtime.switchSpeechBackend(to:)` | Full | `POST /backend` | `switch_speech_backend` | Queues an ordered live backend switch and returns an accepted request. Transport-facing input accepts only `qwen3`, `chatterbox_turbo`, and `marvis`. Pending live transition state is observable from runtime overview/status and the retained request resource. |
...
| `runtime.generate.speech(...)` | Full | `POST /speech/live` | `generate_speech` | Carries `text_profile_id`, `request_context`, `cwd`, `repo_root`, `source_format`, and `qwen_pre_model_text_chunking`. HTTP and MCP speech surfaces fill transport-owned request-context defaults; MCP also folds session `clientInfo` into provenance when available. The Qwen chunking flag is opt-in and defaults to `false`, matching upstream's single-pass Qwen live-playback default when omitted. |
