...
| `runtime.generate.speech(...)` | Full | `POST /speech/live` | `generate_speech` | Carries `text_profile_id`, `request_context`, `cwd`, `repo_root`, and `qwen_pre_model_text_chunking`. HTTP and MCP speech surfaces fill transport-owned request-context defaults; MCP also folds session `clientInfo` into provenance when available. The Qwen chunking flag is opt-in and defaults to `false`, matching upstream's single-pass Qwen live-playback default when omitted. Source-format hints are no longer a speech-submission field; TextForSpeech infers structure from request text and path context. |
