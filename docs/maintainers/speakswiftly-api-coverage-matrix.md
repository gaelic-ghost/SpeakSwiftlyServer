...
| `runtime.generate.speech(...)` | Full | `POST /speech/live` | `generate_speech` | Carries `text_profile_id`, `request_context`, `cwd`, `repo_root`, `source_format`, and `qwen_pre_model_text_chunking`. The Qwen chunking flag is opt-in and defaults to `false`, matching upstream's single-pass Qwen live-playback default when omitted. |
